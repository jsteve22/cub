...
struct Model load_model(const char* filename, struct Metadata data);
i64* model_predict(struct Model model, i64** input, i64* channels, i64* width, i64* height);
...
int main() {
  ...
  /*
  struct Model model = load_model("./miniONN_cifar_model/summary.txt", data);

  fprintf(stdout, "model name: %s\n", model.name);
  fprintf(stdout, "model layer count: %lu\n\n", model.num_layers);
  i64* results = model_predict(model, image, &channels, &width, &height);
  ...
  */
  ...
  /*
  image = conv_layer("./4_layer_cifar_model/conv2d.kernel.txt", image, &channels, &width, &height, 0, data);
  fprintf(stdout, "Done conv2d\n");
  fprintf(stdout, "(%ld, %ld, %ld)\n", channels, width, height);
  image = conv_layer("./4_layer_cifar_model/conv2d_1.kernel.txt", image, &channels, &width, &height, 0, data);
  fprintf(stdout, "Done conv2d_1\n");
  fprintf(stdout, "(%ld, %ld, %ld)\n", channels, width, height);
  image = conv_layer("./4_layer_cifar_model/conv2d_2.kernel.txt", image, &channels, &width, &height, 0, data);
  fprintf(stdout, "Done conv2d_2\n");
  fprintf(stdout, "(%ld, %ld, %ld)\n", channels, width, height);
  image = conv_layer("./4_layer_cifar_model/conv2d_3.kernel.txt", image, &channels, &width, &height, 0, data);
...
struct Model load_model(const char* filename, struct Metadata data) {
  ...
  char *model_name = (char*) malloc(sizeof(char)*256);
  u64 num_layers;

  // use the first line to get information about the rest of the model
  fscanf(fptr, "%s %lu\n", model_name, &num_layers);
  // fprintf(stdout, "model name: %s\n", model_name);

  model.name = model_name;
  model.num_layers = num_layers;
  model.layers = (struct Layer*) malloc( sizeof(struct Layer) * num_layers);

  char *layer_name = (char*) malloc(sizeof(char)*256);

  u64 i;
  for (i = 0; i < num_layers; i++) {
    fscanf(fptr, "%s", layer_name);

    if (strcmp(layer_name, "conv2d") == 0) {
      model.layers[i].type = conv2d;
      model.layers[i].path = (char*) malloc(sizeof(char)*256);
      model.layers[i].activation = (char*) malloc(sizeof(char)*256);
      model.layers[i].bias_path = (char*) malloc(sizeof(char)*256);
      fscanf(fptr, "%s %s %lu %lu %s\n", 
        model.layers[i].path,
        model.layers[i].activation, 
        &(model.layers[i].padding),
        &(model.layers[i].stride),
        model.layers[i].bias_path
      );
    }

    if (strcmp(layer_name, "dense") == 0) {
      model.layers[i].type = dense;
      model.layers[i].path = (char*) malloc(sizeof(char)*256);
      model.layers[i].activation = (char*) malloc(sizeof(char)*256);
      model.layers[i].bias_path = (char*) malloc(sizeof(char)*256);
      fscanf(fptr, "%s %s %s\n", 
    ...
    if (strcmp(layer_name, "meanpooling") == 0) {
      model.layers[i].type = meanpooling;
      fscanf(fptr, "%lu %lu %lu\n", 
        &(model.layers[i].shape),
        &(model.layers[i].padding),
        &(model.layers[i].stride)
      );
    }

    if (strcmp(layer_name, "flatten") == 0) {
      model.layers[i].type = flatten;
    }
  }
...
i64* model_predict(struct Model model, i64** image, i64* channels, i64* width, i64* height) {
  i64* dense_output;

  u64 i;
  for (i = 0; i < model.num_layers; i++) {
    if ( model.layers[i].type == conv2d) { 
      image = conv_layer(model.layers[i].path, image, channels, width, height, model.layers[i].padding, model.data);
    }

    if (model.layers[i].type == dense) {
      dense_output = dense_layer(model.layers[i].path, image, channels, width, height, model.data);
    }

    if (model.layers[i].type == meanpooling) {
      mean_pool_images(image, channels, width, height);
    }

    if (model.layers[i].type == flatten);
