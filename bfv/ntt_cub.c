...
int main() {
  ...
  i64 *private_key = ntt_private_key_generate(size, mod, psi_powers);
  struct PublicKey public_key = ntt_public_key_generate(private_key, size, mod, psi_powers);
  ...
  data.psi = psi;
  data.invpsi = invpsi;
  data.psi_powers = psi_powers;
  data.invpsi_powers = invpsi_powers;
...
i64** conv_layer(const char* filename, i64** input, i64* channels, i64* width, i64* height, i64 padded, struct Metadata data) {
  ...
  i64 *psi_powers = data.psi_powers;
  i64 *invpsi_powers = data.invpsi_powers;
  ...
  for (i = 0; i < *channels; i++) {
    ...
    ct_images[i] = ntt_encrypt(extended_image, public_key, size, mod, plaintext_mod, psi_powers);
  }
  free(extended_image);
  ...
  for (chan = 0; chan < *channels; chan++) {
    for (i = 0; i < ff/filters_per_ciphertext; i++) {
      ...
      temp_a = ntt_ciphertext_plaintext_poly_mult( ct_images[chan], remapped_filter, size, mod, psi_powers);
      temp_b = ct_outputs[i];
  ...
  for (i = 0; i < ff; i+=filters_per_ciphertext) {
    outputs[i] = ntt_decrypt( ct_outputs[i/filters_per_ciphertext], private_key, size, mod, plaintext_mod, invpsi_powers);
...
i64* dense_layer(const char* filename, i64** input, i64* channels, i64* width, i64* height, struct Metadata data) {
  ...
  */
  ...
  for (i = 0; i < ciphertexts_per_vector; i++) {
    ...
    ct_inputs[i] = ntt_encrypt(vector_to_encrypt, public_key, size, mod, plaintext_mod, psi_powers);
  }
  ...
  for (i = 0; i < fc; i++) {
    decrypted_output = ntt_decrypt(ct_outputs[i], private_key, size, mod, plaintext_mod, invpsi_powers);
...
struct Ciphertext __dense_layer(struct Ciphertext* ct_inputs, i64* weights, i64 weights_size, i64 input_size, struct Metadata data) {
  ...
  free(temp_slice);
  output = ntt_ciphertext_plaintext_poly_mult( ct_inputs[ ct_index ], reversed_slice, size, mod, psi_powers);
  ...
  for (ct_index = 1; ct_index < ciphertext_count; ct_index++) {
    ...
    memcpy(reversed_slice, temp_slice, sizeof(i64) * input_size);
    free(temp_slice);
    temp_a = ntt_ciphertext_plaintext_poly_mult( ct_inputs[ ct_index ], reversed_slice, size, mod, psi_powers);
    temp_b = output;
